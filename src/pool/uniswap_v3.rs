...
impl UniswapV3Pool {
    ...
    */
    ...
    pub async fn simulate_swap_with_cache<M: Middleware>(
        ...
        //TODO: look at other options for making this more efficient
        //Look at options to make this more readable, right now we are converting the tick_to_tick_data into an iterator, which to access values like liquidity net, requires foo.1.1 instead of a more readable format.
        let (mut tick_to_tick_data, mut block_number) =
            batch_requests::uniswap_v3::get_uniswap_v3_tick_data_batch_request(
                self,
                self.tick,
                zero_for_one,
        ...
        let mut tick_data_iter = tick_to_tick_data.iter();
        ...
        while current_state.amount_specified_remaining > I256::zero() {
            ...
            let next_tick_data = if let Some(tick_data) = tick_data_iter.next() {
                ...
                let (updated_tick_to_tick_data, updated_block_number) =
                    batch_requests::uniswap_v3::get_uniswap_v3_tick_data_batch_request(
                        self,
                        current_state.tick,
                        zero_for_one,
                ...
                //Update the tick data and block number
                tick_to_tick_data = updated_tick_to_tick_data;
                block_number = updated_block_number;
                tick_data_iter = tick_to_tick_data.iter();
            ...
            step.tick_next = *next_tick_data.0;
            ...
            //If the price moved all the way to the next price, recompute the liquidity change for the next iteration
            if current_state.sqrt_price_x_96 == step.sqrt_price_next_x96 {
                if next_tick_data.1 .0 {
                    let mut liquidity_net = next_tick_data.1 .1;
                ...
                let tick_at_sqrt_price = uniswap_v3_math::tick_math::get_tick_at_sqrt_ratio(
                    current_state.sqrt_price_x_96,
                )?;

                //Advance the iterator until the next tick is equal to the tick at sqrt price

                //TODO: pack this into an `advance_iterator` function

                let mut updated_tick = current_state.tick;
                loop {
                    if let Some(tick_data) = tick_data_iter.next() {
                        if *tick_data.0 == tick_at_sqrt_price {
                            dbg!("BREAKING");
                            break;
                        } else {
                            updated_tick = *tick_data.0;
                        }
                    } else {
                        let (updated_tick_to_tick_data, updated_block_number) =
                            batch_requests::uniswap_v3::get_uniswap_v3_tick_data_batch_request(
                ...
                current_state.tick = tick_at_sqrt_price;
            }
